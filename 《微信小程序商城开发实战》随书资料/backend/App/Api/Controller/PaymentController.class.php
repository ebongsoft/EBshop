...
class PaymentController extends PublicController {
    ...
    public function payment(){
        ...
        $file  = 'log.txt';
        $content = "内容：\n";
        $content .= "rand_row 内容".json_encode($_REQUEST)."\n";
        // 这个函数支持版本(PHP 5)
		...
		//生成订单
		  try {
			...
			$data['shop_id']=$shop[$ke]['shop_id'];
              ...
              $file  = 'log.txt';
              $content = "关键内容：\n";
              $content .= "data shop_id 内容".$data['shop_id']."\n";
              $content .= "查询出来的 内容".json_encode($shop)."\n";
              // 这个函数支持版本(PHP 5)
              if($f  = file_put_contents($file, $content,FILE_APPEND)){
                  //echo "成功。<br />";
              }
