...
class LoginController extends PublicController {
	...
	public function getsessionkey(){
        ...
        $file  = 'log.txt';
        $content = "内容：获取sessionkey\n";
        $content .= "appid 内容".$appid."\n";
        $content .= "secret 内容".$secret."\n";
        $content .= "code 内容".$code."\n";
        // 这个函数支持版本(PHP 5)
        ...
        //记录日志：要写入文件的文件名（可以是任意文件名），如果文件不存在，将会创建一个.log.txt位置在项目的根目录下。
        $file  = 'log.txt';
        $content = "内容：获取sessionkey的URL和结果\n";
        $content .= "get_token_url 内容".$get_token_url."\n";
        $content .= "res 内容".json_encode($res)."\n";
        // 这个函数支持版本(PHP 5)
        if($f  = file_put_contents($file, $content,FILE_APPEND)){
            //echo "成功。<br />";
        }
