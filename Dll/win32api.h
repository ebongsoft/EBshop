...
#Define DT_WORDBREAK		0x10		&& 超出边界进行自动换行. 如用SetTextAlign函数设置了 TA_UPDATECP 标志, 这里的设置则无效.
#Define DT_SINGLELINE		0x20		&& 单行文本
#Define DT_EXPANDTABS 		0x40		&& 扩展制表符, 默认制表跨 8 个字符, 可用 DT_TABSTOP 标志改变这项设定.
#Define DT_TABSTOP			0x80		&& 指定新的制表站间距, 采用这个整数的高 8 位
#Define DT_NOCLIP			0x0100		&& 使文本不限制在指定的矩形内
#Define DT_EXTERNALLEADING 	0x0200		&& 计算文本行高度的时候, 使用当前字体的外部间距属性
#Define DT_CALCRECT			0x0400		&& 自动检测容纳文本的矩形的高度和宽度, 并返回高度, 但文本不被绘制
#Define DT_NOPREFIX 		0x0800		&& 禁止 & 为访问键
#Define DT_INTERNAL			0x1000		&& 用系统字体来计算文本高度
#Define DT_EDITCONTROL		0x2000		&& 如果最后一行只是部分可见, 将不被绘制
#Define DT_PATH_ELLIPSIS	0x4000		&& 文本过长时, 中间使用 ... 会尽量保留最后一个 \ 后面的字符
#Define DT_END_ELLIPSIS		0x8000		&& 文本过长时, 尾部使用 ...
#Define DT_MODIFYSTRING		0x10000		&& 省略显示, 只与 DT_PATH_ELLIPSIS、DT_END_ELLIPSIS 联合使用
#Define DT_RTLREADING		0x20000		&& 从右到左绘制文本, 只用于相应语种
#Define DT_WORD_ELLIPSIS	0x40000		&& 截短不符合矩形的正文, 并增加椭圆
#Define DT_HIDEPREFIX		0x100000	&& 隐藏下划线
#Define DT_PREFIXONLY		0x200000	&& 将 & 解释为前缀
